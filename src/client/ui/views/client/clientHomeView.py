...
class ClientHomeView:
    def __init__(self, page: ft.Page):
        ...
        self.page.inject_server = InjectServer(
            self.page.session.get("jwt_token"))

        # 1. Create a Loading State container
        self.loading_container = ft.Container(
            content=ft.Column([
                ft.ProgressRing(width=50, height=50,
                                stroke_width=4, color=ft.Colors.PRIMARY),
                ft.Text("Syncing with SafeProxy server...",
    ...
    def _async_initialize(self):
        ...
        header = ft.Container(
            content=ft.Column([
                ft.Text(f"Welcome, {self.username}", size=32,
                        weight="bold", color=ft.Colors.PRIMARY),
                ft.Text("Configure, manage, and modify your SafeProxy settings and connections.",
                        size=16, color=ft.Colors.PRIMARY),
            ], spacing=5),
            margin=ft.margin.only(bottom=20),
            # always takes full width - even when the window is small
            col={"xs": 12}
        )
    ...
    def build_home_cards(self) -> list[CustomCard]:
        ...
        self.status_text = ft.Text(
            "DISCONNECTED", color=ft.Colors.GREY, weight="bold")
        ...
        conn_controls = ft.Column(
            controls=[
                ...
                ft.Column(
                    controls=[
                        ft.Row(controls=[
                               ft.Text("Status:", weight="bold"), self.status_text, self.loading_ring]),
                        self.toggle_btn,
                    ],
        ...
        protocol_controls = ft.Column(
            controls=[
                ...
                ft.Text("TLS allows data inspection for better security.",
                        size=12, color=ft.Colors.GREY_500),
                ft.Text("Note: TLS Termination requires SafeProxy CA installed.",
        ...
        redirect_controls = ft.Column(
            controls=[
                ...
                ft.Text("Handles invalid host requests automatically.",
                        size=12, color=ft.Colors.GREY_500),
                ft.Text("Note: Google Redirect requires SafeProxy CA installed.",
                        size=11, color=ft.Colors.GREY_500)
            ],
            spacing=15
        )
        ...
        blacklist_controls = ft.Column(
            controls=[
                ...
                ft.Text("The blacklsit feature allows you to block any site you wish!",
                ...
                ft.DataTable(
                    ...
                    rows=[
                        ft.DataRow(
                            cells=[
                                ft.DataCell(
                                    ft.Text("(Example) example.com", color=ft.Colors.GREY_600)),
                                ft.DataCell(
                                    ft.Text("BLOCKED", color=ft.Colors.RED)),
                                ft.DataCell(
                                    ft.Text("Blocked for reason A, B, C...", color=ft.Colors.GREY_600)),
                                ft.DataCell(ft.IconButton(
                                    icon=ft.Icons.DELETE, icon_color=ft.Colors.GREY_600, disabled=True)),
                            ]
                        )
                    ],
                ...
                ft.TextButton("Manage Full Blacklist", icon=ft.Icons.ARROW_FORWARD,
                              icon_color=ft.Colors.PRIMARY, on_click=lambda e: self.page.go("/blacklist")),
            ],
            spacing=15,
            scroll=ft.ScrollMode.AUTO,
        )
    ...
    def handle_toggle_proxy(self, e: ft.ControlEvent):
        ...
        if e.control.value:  # client tries to connect
            self.page.inject_server.start_inject_server(
                change_ui_when_finished=self.set_status)
        else:
            self.page.inject_server.stop(
                change_ui_when_finished=self.set_status)
    ...
    def handle_change_tls_terminate(self, e: ft.ControlEvent):
        ...
        rsp = self.page.auth_handler.set_tls_terminate(
            self.page.session.get("username"), is_tls_terminate)
        if rsp.status == RspStatus.SUCCESS and is_tls_terminate:
            self.show_snackbar(
                "Successfully changed your tunnling preference to TLS Termination", is_error=False)
            return
        if rsp.status == RspStatus.SUCCESS:
            self.show_snackbar(
                "Successfully changed your tunnling preference to TCP tunneling", is_error=False)
            return

        self.show_snackbar(
            f"Failed changeing your tunnling preference - {rsp.fail_Reason.value}")

    def handle_change_google_redirect(self, e: ft.ControlEvent):
        is_google_redirect = True if e.control.value == "redirect" else False
        rsp = self.page.auth_handler.set_google_redirect(
            self.page.session.get("username"), is_google_redirect)
        if rsp.status == RspStatus.SUCCESS and is_google_redirect:
            self.show_snackbar(
                "Successfully changed your redirection preference to Google Redirect", is_error=False)
            return
        if rsp.status == RspStatus.SUCCESS:
            self.show_snackbar(
                "Successfully changed your redirection preference to 502 Bad Gateaway", is_error=False)
            return
    ...
    def initialize_radio_tls_terminate(self):
        self.tls_radio = ft.Radio(
            value="TLS", label="TLS Termination (Secure)", disabled=True)
        ...
        match (rsp.status, is_ca_installed):
            ...
            case (RspStatus.FAIL, False):
                self.radio_tls_terminate.value = "TCP"
                self.show_snackbar(
                    f"Failed fetching your saved tunneling preference - {rsp.fail_reason.value}.\nDefaulting to TCP.", duration=10000)
    ...
    def initialize_radio_google_redirect(self):
        self.redirect_radio = ft.Radio(
            value="redirect", label="Smart Redirect (To Google Search)", disabled=True)
        self.radio_google_redirect = ft.RadioGroup(
            content=ft.Column([
                self.redirect_radio,
                ft.Radio(value="502_rsp",
                         label="Send a 502 'Bad Gateaway' response"),
            ],
                spacing=10
            ),
        ...
        match (rsp.status, is_ca_installed):
            ...
            case (RspStatus.FAIL, False):
                self.radio_google_redirect.value = "502_rsp"
                self.show_snackbar(
                    f"Failed fetching your saved redirection preference - {rsp.fail_reason.value}. Defaulting to 502 Bad Gateaway.")

            # edge-case 6
            case (RspStatus.FAIL, True):
                self.radio_google_redirect.value = "redirect"
                self.show_snackbar(
                    f"Failed fetching your saved redirection preference - {rsp.fail_reason.value}. Defaulting to Google Redirect.")
            ...
            case (RspStatus.SUCCESS, False):
                if rsp.google_redirect:
                    ...
                    self.page.auth_handler.set_google_redirect(
                        self.username, False)

                    self.show_snackbar(
                        f"SafeProxy can't Google Redirect if CA is not installed. If you wish to continue with Google Redirect, please install SafeProxy CA. As things stand, defaulting to 502 Bad Gateaway.")
    ...
    def run_startup_ca_check(self):
        ...
        if rsp.status == RspStatus.SUCCESS:
            if not CAHandler.update_local_file(rsp.ca_cert):
                # we dont know it is actually installed - but need to disable install feature
                self.page.session.set("CA_installed", False)

                self._show_ca_modal(
                    title="SafeProxy CA certificate: Local update error",
                    details="SafeProxy updated it's root CA certificate, used for signing host certificates.\n"
            ...
            if CAHandler.is_ca_cert_installed(rsp.ca_cert):
                ...
                self._show_ca_modal(
                    ...
                    details="SafeProxy updated it's root CA certificate (or your local CA certificate\n"
            ...
            self._show_ca_modal(
                ...
                details="Failed getting SafeProxy's most up-to-date CA certificate, used for signing host \n"
