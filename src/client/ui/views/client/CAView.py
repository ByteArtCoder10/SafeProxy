...
class CAView:
    def __init__(self, page: ft.Page):
        ...
        self.status_text = ft.Text(
            "CHECKING...", color=ft.Colors.GREY, weight="bold")
        ...
        self.error_banner = ft.Text(
            value="Failed installing Certifcate", color=ft.Colors.ERROR, size=14, visible=False)

        self.install_btn = CustomBtn(text="Install SafeProxy CA", bg_color=ft.Colors.GREY,
                                     disabled=True, elevation=2, on_click=self._install_ca)
        ...
        para = CustomCard(ft.Text(text, color=ft.Colors.GREY_700,
                          text_align=ft.TextAlign.START, size=15))
        ...
        if self.page.session.contains_key("CA_installed"):
            self._change_ui_installed() if self.page.session.get(
                "CA_installed") else self._change_ui_not_installed()
