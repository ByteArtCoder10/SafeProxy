...
class BlackListView:
    def __init__(self, page: ft.Page):
        ...
        bl_para = ft.Column(
            controls=[
                ft.Text("SafeProxy can block hosts, URLs, and even IPs!",
                        color=ft.Colors.SECONDARY, weight="bold", size=20),
                ft.Text("The blocking mechanism is as follows:\n "
                        ...
                        "from this host.\n"
                        "For example, blocking example.com/x will block example.com/x but not example.com/y and example.com/", color=ft.Colors.GREY_500, size=14),
        ...
        btns = ft.Row(
            controls=[
                CustomBtn(text="Add Host", on_click=self.add_host_modal.open,
                          bg_color=ft.Colors.WHITE, text_color=ft.Colors.PRIMARY, elevation=2),
                CustomBtn(text="Delete All", on_click=self._open_dialog,
                          bg_color=ft.Colors.WHITE, text_color=ft.Colors.RED, elevation=2),
            ],
        ...
        self.blacklist_table = ft.DataTable(
            columns=[
                ...
                ft.DataColumn(label=ft.Container(
                    ft.Text("Details", weight="bold"), width=400)),
    ...
    def _populate_table(self):
        ...
        rsp = self.page.auth_handler.get_blacklist(
            self.page.session.get("username"))
        if rsp.status == RspStatus.SUCCESS:
            client_logger.info(f"Server's repsonse: {rsp.__dict__}")
            blacklist_dict = rsp.blacklist
        else:
            self.show_snackbar(
                f"Error - Failed fetching your blacklist: {rsp.fail_reason.value}")
            return
                ...
                ft.DataRow(
                    cells=[
                        ft.DataCell(
                            ...
                            ft.Container(
                                content=ft.Text(
                                    host, overflow=ft.TextOverflow.CLIP),
                        ...
                        ft.DataCell(
                            ...
                            ft.Container(
                                content=ft.Text(
                                    details, overflow=ft.TextOverflow.CLIP),
                                width=350,  # Limits width to force wrap
                                padding=ft.padding.symmetric(vertical=5),
                            )
                        ),
                        ft.DataCell(
                            ft.IconButton(
                                ...
                                on_click=lambda e, h=host: self._delete_host(
                                    e, h)
                            )
                        )
                    ]
    ...
    def _add_host(self, e: ft.ControlEvent):
        ...
        # DB request to add
        rsp = self.page.auth_handler.add_blacklist_host(
            self.page.session.get("username"), host, details)
        ...
        if rsp.status == RspStatus.SUCCESS:
            ...
            self.show_snackbar(
                f"Successfully added {host} to your blacklist!", is_error=False)
        else:
    ...
    def _delete_host(self, e: ft.ControlEvent, host: str):
        ...
        rsp = self.page.auth_handler.delete_blacklist_host(
            self.page.session.get("username"), host)

        if rsp.status == RspStatus.SUCCESS:
            self.blacklist_table.rows = [
                row for row in self.blacklist_table.rows if row.cells[0].content.content.value != host]

            self.show_snackbar(
                f"Successfully deleted {host} from your blacklist", is_error=False)
        else:
            self.show_snackbar(f"Error - {rsp.fail_reason.value}",)
    ...
    def _delete_all(self, e=None):
        # delete
        rsp = self.page.auth_handler.delete_full_blacklist(
            self.page.session.get("username"))
        ...
        if rsp.status == RspStatus.SUCCESS:
            ...
            self.show_snackbar(
                "Successfully deleted all blacklist.", is_error=False)
        else:
            self.show_snackbar(f"Error - {rsp.fail_reason.value}")
        self.page.update()
