...
class InjectServer:
    ...
    def _start(self, change_ui_when_finished) -> None:
        ...
        try:
            ...
            client_logger.info(
                f"Server up at ({self._bind_ip},{self._bind_port})")
            client_logger.info(
                f"Forwarding to Proxy at ({self._proxy_ip},{self._proxy_port})")

            # Check if proxy is even reachable, in order for the UI to show a suitable status
            try:
                test_socket = socket.create_connection(
                    (self._proxy_ip, self._proxy_port), timeout=2)
                test_socket.close()
                client_logger.info(
                    "[InjectServer] Test connection to proxy was successfull.")
    ...
    def _run_new_chrome_proccess(self):
        try:
            ...
            client_logger.warning(
                f"Failed to open a dedicated Chrome browser with proxy configuration: {e.stderr}", exc_info=True)
    ...
    def stop(self, change_ui_when_finished=None):
        ...
        try:
            ...
            client_logger.info(
                "[InjectServer] Disconnected from proxy, and shutting off Inject Server.")
            change_ui_when_finished(
                False, "DISCONNECTED") if change_ui_when_finished else None
        except Exception:
            pass
        ...
        try:
            ...
            try:
                ...
                if parsed_req:
                    ...
                    client_logger.info(
                        "Added Proxy-Auth header to client's request.")
                ...
                client_logger.critical(
                    f"Parsing error, sending without Proxy-Auth-header: {e}", exc_info=True)
                modified_data = request_data
            ...
            client_logger.error(
                f"Connection Handler Error: {e}", exc_info=True)
        finally:
            if client_socket:
                client_socket.close()
            if proxy_socket:
    ...
    def _run_tunnel_relay(self, client_socket: socket.socket, proxy_socket: socket.socket):
        ...
        # InjectServer -> Proxy
        t1 = threading.Thread(target=_relay_data, args=(
            client_socket, proxy_socket))
        t1.daemon = True

        # Proxy -> InjectServer
        t2 = threading.Thread(target=_relay_data, args=(
