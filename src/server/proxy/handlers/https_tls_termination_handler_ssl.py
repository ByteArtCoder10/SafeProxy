...
class HttpsTlsTerminationHandlerSSL(BaseHandler):
    ...
    def process(self, req: Request, client_socket: socket, googleSearchRedirect: bool):
        ...
        try:
            ...
            self._respond_to_client(
                req, self._client_socket, 200, isConnectionEstablished=True)
            core_logger.debug(
                f"TIMECHECK0 - {datetime.datetime.now() - before}")
            before = datetime.datetime.now()
            ...
            client_hello_data = self._recvall(
                client_hello_length, msg_peek=True)
            sni = self._extract_sni_from_client_hello(client_hello_data)
            if not sni:
                sni = req.host

            core_logger.debug(
                f"TIMECHECK1 - {datetime.datetime.now() - before}")
            before = datetime.datetime.now()

            # Create a 'on-the-fly' cert
            cert_path, key_path = self._ca_authority.get_certificate_for_host(
                sni)  # if sno wasnt found - host
            core_logger.debug(
                f"TIMECHECK2 - {datetime.datetime.now() - before}")
            before = datetime.datetime.now()
            ...
            core_logger.debug(
                f"TIMECHECK3 - {datetime.datetime.now() - before}")
            before = datetime.datetime.now()
            ...
            core_logger.debug(
                f"TIMECHECK4 - {datetime.datetime.now() - before}")
            before = datetime.datetime.now()

            # resume tls handshake
            raw_request = self._resume_tls_conenction()
            core_logger.debug(
                f"TIMECHECK5 - {datetime.datetime.now() - before}")
            before = datetime.datetime.now()
            ...
            if self.url_manager.is_blacklisted([req.host + req.path, sni], self._username):
                self._respond_to_client(
                    req, self._tls_client_connection, 403, addBlackListLabelHTML=True)
                return
            ...
            client_request = Parser.parse_request(
                raw_request.decode(encoding="utf-8", errors="ignore"))

            # check raw request again
            if self.url_manager.is_blacklisted([client_request.host + client_request.path, sni], self._username):
                self._respond_to_client(
                    client_request, self._tls_client_connection, 403, addBlackListLabelHTML=True)
                return
            # if self.url_manager.is_malicious(sni):
            ...
            handshake_success = self._perform_tls_hanshake_with_server(
                client_request, sni)
            core_logger.debug(
                f"TIMECHECK6 - {datetime.datetime.now() - before}")
            before = datetime.datetime.now()

            if not handshake_success:
                if googleSearchRedirect:
                    core_logger.info(
                        f"Failed connecting to server ({sni}). Redirecting to google search.")
                    self._respond_to_client(
                        req, self._tls_client_connection, 200, redirectURL=UrlManager.get_google_url(sni))
                    return
                else:
                    core_logger.info(
                        f"Failed connecting to server ({sni}). Sending 502 Bad Gateaway.")
                    self._respond_to_client(
                        req, self._tls_client_connection, 502, addBlackListLabelHTML=True)
                    return
            ...
            core_logger.debug(
                f"TIMECHECK7 - {datetime.datetime.now() - before}")
            before = datetime.datetime.now()

            self._run_tunnel_relay()
            core_logger.debug(
                f"TIMECHECK8 - {datetime.datetime.now() - before}")
    ...
    def _resume_tls_conenction(self) -> bytes | None:
        ...
        try:
            ...
            if not raw_request:
                core_logger.debug(
                    "Client closed TLS connection without sending data")
                return None

            core_logger.debug(
                f"Cient's request after TLS termination: \n{raw_request.decode(encoding="utf-8", errors="replace")}")
            return raw_request
        ...
        # Client/Proxy closed the connection
            ...
            core_logger.debug(
                "Connection timed out - client didn't send any request.")
            return None
    ...
    def _perform_tls_hanshake_with_server(self, req: Request, sni: str) -> bool:
        ...
        try:
            ...
            self._tls_server_connection = self._server_ssl_context.wrap_socket(
                self._server_socket, server_hostname=sni)
            core_logger.info(
                f"Successfully and securely connected to server {req.host}.")
            return True
    ...
    def _run_tunnel_relay(self):
        ...
        t1 = threading.Thread(
            ...
            args=(self._tls_client_connection, self._tls_server_connection,
                  ProxyContext.get_local().__dict__),
            daemon=True
        )
    ...
    def _handle_relay_data(self, recv_socket: ssl.SSLSocket, send_socket: ssl.SSLSocket, local_thread_vars: dict):
        ...
        ProxyContext.set_local(
            host=local_thread_vars["host"], ip=local_thread_vars["ip"], port=local_thread_vars["port"])
    ...
    def _relay_data(self, recv_socket: ssl.SSLSocket, send_socket: ssl.SSLSocket):
        ...
        try:
            ...
            core_logger.error(
                f"Failed setting a timeout for the socket: {e}", exc_info=True)
