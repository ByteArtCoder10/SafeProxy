...
class HttpsTcpTunnelHandler(BaseHandler):
    ...
    def process(self, req, client_socket, googleSearchRedirect: bool):
        ...
        if self.url_manager.is_blacklisted([url], self._username):
            core_logger.info(
                "URL requested is blacklisted. TLS-Terminating and sending 403 blacklisted.")
            HttpsTlsTerminationHandlerSSL(self._ca_authority).handle(
                req, client_socket, self._username, googleSearchRedirect)
            return
        ...
        try:
            ...
            match conn_status:
                ...
                case ConnectionStatus.REDIRECT_REQUIRED:
                    core_logger.debug(
                        f"Connection failed for {req.host}. TLS terminating and Redirecting to Google.")
                ...
                case ConnectionStatus.CONNECT_FAILURE:
                    core_logger.info(
                        f"Connection failed for {req.host}. TLS-Terminating and Sending 502.")
