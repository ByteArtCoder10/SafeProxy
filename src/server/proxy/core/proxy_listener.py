...
class ProxyListener:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            while True:
                ...
                client_thread.start()
                core_logger.info(
                    f"Thread started for client - {client_address}")
            ...
            core_logger.warning(
                f"Proxie's listener interuppted: {e}", exc_info=True)
    ...
    def handle_client_request(self, client_socket: socket.socket, client_address: tuple[str, int]) -> None:
        ...
        try:
            ...
            client_request = client_socket.recv(
            ...
            core_logger.debug(
                f"A request from {client_address}:\n{client_request}")
            ...
            jwt_valid, username = AuthValidator.is_request_authorized(
                parsed_request, self.auth_public_key)
            if not jwt_valid or not username:
                ...
                ProxyContext.set_local(
                    "BLOCKED", client_address[0], client_address[1])
            ...
            core_logger.debug(
                f"Changed local variables: replaced 'PENDING' with {parsed_request.host}")

            # Route based on request
            self._router.route_request(
                parsed_request, client_socket, self._ca, username)

        except socket.timeout:
            core_logger.warning(
                "conenction timed-out: Client connected, but never sent a request.")
